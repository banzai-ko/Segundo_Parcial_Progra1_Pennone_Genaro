...
class MenuEnterName(Menu):
    ...
    def __init__(self, name: str, pantalla: object, x: int, y: int, active: bool, level_num: int, music_name: str, score: int) -> None:
        ...
        self.surface = pg.image.load(
            './game/assets/img/forms/form_enter_name.png').convert_alpha()
        ...
        self.title = TextTitle(x=DIMENSION_PANTALLA[0]//2, y=DIMENSION_PANTALLA[1] //
                               2-200, texto="This or that", pantalla=pantalla, font_size=75)
        self.subtitle = TextTitle(x=DIMENSION_PANTALLA[0]//2, y=DIMENSION_PANTALLA[1] //
                                  2-90, texto="INGRESE SU NOMBRE:", pantalla=pantalla, font_size=50)
        self.subtitle_score = TextTitle(
            x=DIMENSION_PANTALLA[0]//2, y=DIMENSION_PANTALLA[1]//2-20, texto=f"PUNTAJE:{score}", pantalla=pantalla, font_size=30)

        self.text_box = TextBox(
            x=DIMENSION_PANTALLA[0]//2, y=DIMENSION_PANTALLA[1]//2+40, texto="_________________", pantalla=pantalla)
        self.button_confirm_name = Button(x=DIMENSION_PANTALLA[0]//2, y=DIMENSION_PANTALLA[1] //
                                          2+100, texto="CONFIRMAR NOMBRE", pantalla=pantalla, on_click=self.click_confirm_name)

        self.widget_list = [self.title, self.subtitle,
                            self.subtitle_score, self.button_confirm_name]
